...
class VisualMaster:
    ...
    def animate_header(cls, user_level="USER"):
        ...
        # ══ PHASE 3: PHOBEX ASCII art (typed letter-by-letter effect) ════
        PHOBEX = [
            " ██████╗ ██╗  ██╗██████╗ ██████╗ ███████╗██╗  ██╗ ",
            " ██╔══██╗██║  ██║██╔══██╗██╔═══██╗██╔════╝╚██╗██╔╝ ",
            " ██████╔╝███████║██║  ██║██║   ██║█████╗   ╚███╔╝  ",
        ...
        for i, line in enumerate(PHOBEX):
            shade = SH[i % len(SH)]
        ...
        for i, line in enumerate(XALINAS):
            shade = SH[(i+3) % len(SH)]
            for end in range(1, len(line)+1, 4):
                sys.stdout.write(f"\r{shade}{B}{line[:end].center(W)}{R}")
                sys.stdout.flush()
        ...
        spin = ["◐","◓","◑","◒","◐","◓","◑","◒","●","○","●"]
        for s in spin:
            sys.stdout.write(f"\r{C['c1']}{B}  {s}  PHOBEX & xAlInAs  {s}  {R}")
            sys.stdout.flush()
            time.sleep(0.07)
        print()
...
class Config:
    ...
    CUSTOM_NAME_PREFIX = "xAlInAs_sAnIlAx"
    CUSTOM_PASS_PREFIX = "xAlInAs_sAnIlAx"
    ...
    BIO_TEXT = """[b][c][FF0000] █▀▀▀ [00ff90] ▄█░ ▄▀▀▄[ff0000]INSTA:[00ffff]xalinas_sanilax
[b][c][FF0000] █░▀█ [00ff90] ░█░ ▄▀▀▄[ff0000]TIK TOK :[00ffff]xalinas_sanilax
[b][c][FF0000] ▀▀▀▀[00ff90] ▄█▄ ▀▄▄▀"""
    ...
    ACTIVATION_REGIONS = {
        'IND': {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
        ...
        'BD':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
        'PK':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
        'ID':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
        'TH':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.common.ggbluefox.com/GetLoginData',
                'client_host': 'clientbp.common.ggbluefox.com'},
        'VN':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
        'ME':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.common.ggbluefox.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
        'BR':  {'guest_url': 'https://100067.connect.garena.com/oauth/guest/token/grant',
                'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin',
                'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData',
                'client_host': 'clientbp.ggpolarbear.com'},
    }
    ...
    # NEW: /api/v2/ endpoint + JSON body (from OB54new.zip main.py)
    ...
    BASE_FOLDER            = os.path.join(CURRENT_DIR, "xAlInAs-GEN")
...
def generate_random_name():
    base = Config.CUSTOM_NAME_PREFIX if Config.CUSTOM_NAME_PREFIX else "xAlInAs_sAnIlAx"
...
def generate_custom_password():
    """OB54: full 64-char hex password"""
...
class CreditEditor:
    ...
    def load_credits(cls):
        default_credits = {
            "primary_credit": "xAlInAs",
            "telegram1": "@xAlInAs_sAnIlAx",
            "CH PHOBEX TEAM": "@phobex",
            "display_name": "xAlInAs",
            "banner_text": "⚡ POWERED BY  ALINAS ⚡",
            "footer_text": "👤 CREDIT: xAlInAs | ch: @phobex",
            "bio_text": """[b][c][FF0000] █▀▀▀ [00ff90] ▄█░ ▄▀▀▄[ff0000]INSTA:[00ffff]xalinas_sanilax
[b][c][FF0000] █░▀█ [00ff90] ░█░ ▄▀▀▄[ff0000]TIK TOK :[00ffff]xalinas_sanilax
[b][c][FF0000] ▀▀▀▀[00ff90] ▄█▄ ▀▄▄▀""",
...
_BIO_SERVERS = {
    ...
    "PK":  "https://clientbp.ggpolarbear.com/UpdateSocialBasicInfo",
    "ID":  "https://clientbp.ggpolarbear.com/UpdateSocialBasicInfo",
    "VN":  "https://clientbp.ggpolarbear.com/UpdateSocialBasicInfo",
...
class AutoActivator:
    ...
    def major_login(self, access_token, open_id, region='IND'):
        ...
        headers = {
            'X-Unity-Version': '1.128.14', 'ReleaseVersion': 'OB54',
        ...
        OLD_OPEN_ID      = b"dec2e823a7f0770c887ef1ca4d011c0c"
        OLD_ACCESS_TOKEN = b"f880f01893fc782df0c9586ebeb22a4c6c94da62cc453e01f34e658a8095af70"
    ...
    def GET_LOGIN_DATA(self, JWT_TOKEN, access_token, region='IND'):
        ...
        try:
            ...
            payload = payload.replace(b"f880f01893fc782df0c9586ebeb22a4c6c94da62cc453e01f34e658a8095af70", access_token.encode("UTF-8"))
        ...
        headers = {
            ...
            'X-Unity-Version': '1.128.14', 'X-GA': 'v1 1', 'ReleaseVersion': 'OB54',
            'Content-Type': 'application/x-www-form-urlencoded',
...
def create_acc(region, session, is_ghost=False):
    """
    OB54 guest registration using the NEW /api/v2/ JSON endpoints.
...
def _major_register_and_login_sync(uid, password, access_token, open_id, name,
    ...
    try:
        ...
        headers_reg = {
            ...
            "X-GA": "v1 1", "X-Unity-Version": "1.128.14",
        }
...
def _encrypt_major_login_proto(open_id, access_token):
    """
    OB54 NEW: Build MajorLogin payload using MajoRLoGinrEq_pb2 protobuf
...
def _perform_major_login_sync(uid, password, access_token, open_id, region, session, is_ghost=False):
    """
    OB54 UPDATED: Uses new protobuf MajorLogin (genapi.zip) as primary method,
...
def _force_region_binding(region, jwt_token, session):
    try:
        ...
        headers = {
            ...
            'Authorization': f"Bearer {jwt_token}", 'X-Unity-Version': "1.128.14",
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        entry = {
            ...
            'region': "xAlInAs_sAnIlAx" if is_ghost else region,
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    ...
    try:
        ...
        entry = {
            'uid': account_data["uid"], 'password': account_data["password"],
            'account_id': account_data.get("account_id","N/A"), 'name': account_data["name"],
            'region': "xAlInAs_sAnIlAx" if is_ghost else account_data.get('region','UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        entry = {
            ...
            'reason': reason, 'region': "xAlInAs_sAnIlAx" if is_ghost else region,
...
19) ⬅️  Back"""
        ...
        if choice == "1":
            ...
            d = {"primary_credit":"ALINAS","PHOBEX TEAM":"@phobex",
                 "telegram1":"@xAlInAs_sAnIlAx","CH ":"@x_5_c_h","display_name":"ALS",
                 "banner_text":"⚡ POWERED BY xAlInAs ⚡",
                 "footer_text":"👤 CREDIT: xAlInAs | ch : @phobex",
...
def about():
    VISUAL.show_header(Config.USER_LEVEL)
    credits = CreditEditor.load_credits()
    about_text = f"""🔥 {credits['display_name']} ULTIMATE GENERATOR V2 By @x0AlInAs
